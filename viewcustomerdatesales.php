                                ...
                                <table id="example1" class="table table-bordered table-striped">
									...
									<tbody>
									     <?php
                                            ...
                                            {
                                                $id=$_POST['action_id'];
                                                $sql = "SELECT vendor_sell.*,customer.name AS cname,customer.pending_amount as pa FROM vendor_sell,customer WHERE `customer_id`='$id' AND vendor_sell.customer_id IN (SELECT id from customer GROUP BY name) AND vendor_sell.customer_id=customer.id  ORDER by date DESC";
                                            }
                                            else
                                            {
                                                $sql = "SELECT vendor_sell.*,customer.name AS cname,customer.pending_amount as pa FROM vendor_sell,customer WHERE vendor_sell.customer_id IN (SELECT id from customer GROUP BY name) AND vendor_sell.customer_id=customer.id  ORDER by date DESC";
                                            }
